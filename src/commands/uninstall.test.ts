...
vi.mock("../constants.js", () => ({
	get MANIFEST_PATH() {
		return getUninstallPaths().MANIFEST_PATH;
	},
	get BACKUP_DIR() {
		return getUninstallPaths().BACKUP_DIR;
	},
...
describe("buildUninstallPlan", () => {
	beforeEach(async () => {
		currentTmpDir = path.join(
			os.tmpdir(),
			`javi-ai-uninstall-${crypto.randomUUID()}`,
		);
	...
	it("opencode in manifest: includes skills-dir, config-section, NO hooks-dir", async () => {
		...
		await fs.ensureDir(configSrc);
		await fs.writeFile(
			path.join(configSrc, "OPENCODE.md"),
			"# opencode",
			"utf-8",
		);
		await fs.ensureDir(p.OPENCODE_CONFIG);
		await fs.writeFile(
			path.join(p.OPENCODE_CONFIG, "OPENCODE.md"),
			"# installed",
			"utf-8",
		);
...
describe("runUninstall", () => {
	...
	it("skills-dir item: directory is removed", async () => {
		...
		await fs.ensureDir(p.CLAUDE_SKILLS);
		await fs.writeFile(
			path.join(p.CLAUDE_SKILLS, "skill.md"),
			"content",
			"utf-8",
		);
	...
	it("hooks-dir: only removes files from javi-ai hooks source; user-created hooks survive", async () => {
		...
		await fs.writeFile(
			path.join(hooksDir, "user-hook.sh"),
			"#!/bin/sh",
			"utf-8",
		);
		...
		expect(await fs.pathExists(path.join(hooksDir, "post-tool.sh"))).toBe(
			false,
		);
	...
	it("config-section no backup, has markers: markers and content stripped, surrounding preserved", async () => {
		...
		expect(result.removed.some((r) => r.includes("section removed"))).toBe(
			true,
		);
	});
	...
	it("config-section with non-.md file is skipped (no config-section for .json in buildUninstallPlan)", async () => {
		...
		await fs.ensureDir(p.OPENCODE_CONFIG);
		await fs.writeFile(
			path.join(p.OPENCODE_CONFIG, "opencode.json"),
			"{}",
			"utf-8",
		);
	...
	it("buildUninstallPlan: skills-dir item path equals skillsPath", async () => {
		...
		const { items } = await buildUninstallPlan();
		const skillsItem = items.find(
			(i) => i.type === "skills-dir" && i.cli === "claude",
		);
	...
	it("buildUninstallPlan: README.md is excluded from config-section items", async () => {
		...
		await fs.ensureDir(p.CLAUDE_CONFIG);
		await fs.writeFile(
			path.join(p.CLAUDE_CONFIG, "README.md"),
			"# README",
			"utf-8",
		);
		await fs.writeFile(
		...
		expect(configSectionItems.every((i) => !i.path.includes("README.md"))).toBe(
			true,
		);
		// CLAUDE.md should be there
		expect(configSectionItems.some((i) => i.path.includes("CLAUDE.md"))).toBe(
			true,
		);
	});
	...
	it('config-section: no markers → file left untouched, item in removed with "skipped"', async () => {
		...
		expect(
			result.removed.some(
				(r) => r.includes("skipped") || r.includes("no javi-ai markers"),
			),
		).toBe(true);
	});
	...
	it("runUninstall: section join with \\n\\n (not empty string)", async () => {
		...
		expect(remaining.indexOf("# Footer")).toBeGreaterThan(
			remaining.indexOf("# Header") + "# Header".length,
		);
	});
	...
	it("runUninstall hooks: removed path contains hook filename with /", async () => {
		...
		await fs.ensureDir(hooksDir);
		await fs.writeFile(
			path.join(hooksDir, "javi-hook.sh"),
			"#!/bin/sh",
			"utf-8",
		);
		...
		await fs.ensureDir(hooksSrc);
		await fs.writeFile(
			path.join(hooksSrc, "javi-hook.sh"),
			"#!/bin/sh",
			"utf-8",
		);
