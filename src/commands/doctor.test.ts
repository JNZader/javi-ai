...
vi.mock("../constants.js", () => ({
	get MANIFEST_PATH() {
		return getDoctorPaths().MANIFEST_PATH;
	},
	get BACKUP_DIR() {
		return getDoctorPaths().BACKUP_DIR;
	},
...
describe("runDoctor", () => {
	beforeEach(async () => {
		currentTmpDir = path.join(
			os.tmpdir(),
			`javi-ai-doctor-${crypto.randomUUID()}`,
		);
	...
	it("manifest with 2 CLIs: shows them listed", async () => {
		...
		const cliCheck = installSection.checks.find((c) =>
			c.label.includes("CLIs configured"),
		);
	...
	it("CLI in manifest + found in PATH: cli status = ok", async () => {
		...
		const cliSection = result.sections.find(
			(s) => s.title === "CLI Detection",
		)!;
		const claudeCheck = cliSection.checks.find(
			(c) => c.label.trim() === "claude",
		)!;
	...
	it("skill count matches expected: status ok", async () => {
		...
		await fs.writeFile(
			path.join(upstreamSkills, "react-19", "SKILL.md"),
			"# React 19",
			"utf-8",
		);
		...
		await fs.writeFile(
			path.join(p.CLAUDE_SKILLS, "react-19", "SKILL.md"),
			"# React 19",
			"utf-8",
		);
		...
		const claudeSkillCheck = skillSection.checks.find(
			(c) => c.label.trim() === "claude",
		)!;
	...
	it("skill count below expected: status fail", async () => {
		...
		for (const s of ["react-19", "typescript"]) {
			...
			await fs.writeFile(
				path.join(upstreamSkills, s, "SKILL.md"),
				`# ${s}`,
				"utf-8",
			);
		}
	...
	it("config file present: section has ok/fail checks (not skip) for installed CLI", async () => {
		...
		const claudeConfigCheck = configSection.checks.find(
			(c) => c.label.includes(".claude") || c.label.includes("CLAUDE"),
		);
	...
	it("hook file absent: status fail (when hooks source exists)", async () => {
		...
		await fs.writeFile(
			path.join(hooksSrc, "check-format.sh"),
			"#!/bin/sh",
			"utf-8",
		);
		...
		const hookCheck = hookSection.checks.find((c) =>
			c.label.includes("check-format.sh"),
		);
		if (hookCheck) {
	...
	it("hook file present + executable: hook check status reflects install state", async () => {
		...
		const hookCheck = hookSection.checks.find(
			(c) => c.label === "post-tool.sh",
		);
	...
	it("backup dirs listed (up to 5)", async () => {
		...
		const moreEntry = backupSection.checks.find((c) =>
			c.label.includes("more"),
		);
	...
	it("backup file count is correct (no double-counting) after bug fix", async () => {
		...
		await fs.writeFile(
			path.join(backupDir, "subdir", "nested.md"),
			"d",
			"utf-8",
		);
	...
	it("backup dirs are sorted newest-first (reverse chronological)", async () => {
		...
		for (const name of backupNames) {
			...
			await fs.writeFile(
				path.join(p.BACKUP_DIR, name, "file.md"),
				"x",
				"utf-8",
			);
		}
	...
	it("skill count: installed skills WITHOUT SKILL.md are not counted", async () => {
		...
		await fs.writeFile(
			path.join(p.CLAUDE_SKILLS, "fake-skill", "README.md"),
			"# Fake",
			"utf-8",
		);
	...
	it('empty backup directory: shows "No backups found" skip check', async () => {
		...
		const result = await runDoctor();
		const backupSection = result.sections.find((s) => s.title === "Backups")!;
		const skipCheck = backupSection.checks.find(
			(c) => c.label.includes("No backups") && c.status === "skip",
		);
		expect(skipCheck).toBeDefined();
	});
	...
	it('backup count >5: shows "and N more" with correct count', async () => {
		...
		for (let i = 1; i <= 8; i++) {
			...
			await fs.ensureDir(path.join(p.BACKUP_DIR, name));
			await fs.writeFile(
				path.join(p.BACKUP_DIR, name, "test.md"),
				"# test",
				"utf-8",
			);
		}
	...
	it("countExpectedSkills: same skill name in own/ and upstream/ is only counted once", async () => {
		...
		await fs.writeFile(
			path.join(upstreamSkills, "typescript", "SKILL.md"),
			"# TS",
			"utf-8",
		);
		...
		await fs.writeFile(
			path.join(ownSkills, "react-19", "SKILL.md"),
			"# React 19 own",
			"utf-8",
		);
		...
		await fs.writeFile(
			path.join(p.CLAUDE_SKILLS, "typescript", "SKILL.md"),
			"# TS",
			"utf-8",
		);
		...
		const result = await runDoctor();
		const skillSection = result.sections.find((s) => s.title === "Skills")!;
		const claudeCheck = skillSection.checks.find(
			(c) => c.label.trim() === "claude",
		)!;
	...
	it("countExpectedSkills: upstream skills without SKILL.md are excluded", async () => {
		...
		await fs.writeFile(
			path.join(upstreamSkills, "skill-without-md", "README.md"),
			"# Not counted",
			"utf-8",
		);
		...
		await fs.writeFile(
			path.join(upstreamSkills, "valid-skill", "SKILL.md"),
			"# Valid",
			"utf-8",
		);
		...
		await fs.writeFile(
			path.join(p.CLAUDE_SKILLS, "valid-skill", "SKILL.md"),
			"# Valid",
			"utf-8",
		);
	...
	it("countExpectedSkills: upstream dot-dirs and _shared are excluded", async () => {
		...
		await fs.writeFile(
			path.join(upstreamSkills, ".hidden", "SKILL.md"),
			"# Hidden",
			"utf-8",
		);
		...
		await fs.writeFile(
			path.join(upstreamSkills, "_shared", "SKILL.md"),
			"# Shared",
			"utf-8",
		);
		// real skill
	...
	it("which returns null (empty stdout): cli status is fail", async () => {
		...
		mockExecFile.mockImplementation(
			(_bin: string, _args: unknown, callback: unknown) => {
				// eslint-disable-next-line @typescript-eslint/no-explicit-any
	...
	it("skill count: present === expected → ok (not fail)", async () => {
		...
		await fs.writeFile(
			path.join(upstreamSkills, "the-skill", "SKILL.md"),
			"# The Skill",
			"utf-8",
		);
		...
		await fs.writeFile(
			path.join(p.CLAUDE_SKILLS, "the-skill", "SKILL.md"),
			"# The Skill",
			"utf-8",
		);
	...
	it("skill count: present === expected - 1 → fail", async () => {
		...
		await fs.writeFile(
			path.join(upstreamSkills, "skill-a", "SKILL.md"),
			"# A",
			"utf-8",
		);
		await fs.ensureDir(path.join(upstreamSkills, "skill-b"));
		await fs.writeFile(
			path.join(upstreamSkills, "skill-b", "SKILL.md"),
			"# B",
			"utf-8",
		);
		...
		await fs.writeFile(
			path.join(p.CLAUDE_SKILLS, "skill-a", "SKILL.md"),
			"# A",
			"utf-8",
		);
	...
	it("sections contain correct status distribution: ok/fail/skip mix", async () => {
		...
		const installOkChecks = installSection.checks.filter(
			(c) => c.status === "ok",
		);
		...
		const claudeCliCheck = cliSection.checks.find(
			(c) => c.label.trim() === "claude",
		)!;
		const opencodeCliCheck = cliSection.checks.find(
			(c) => c.label.trim() === "opencode",
		)!;
	...
	it("config section: installed CLI with missing config file → fail", async () => {
		...
		const claudeConfig = configSection.checks.find(
			(c) => c.label.includes("CLAUDE.md") || c.label.includes(".claude"),
		)!;
	...
	it("config section: opencode installed → shows ok or fail (not skip)", async () => {
		...
		const opencodeConfig = configSection.checks.find((c) =>
			c.label.includes("opencode"),
		);
		if (opencodeConfig) {
	...
	it("hook check: hook file exists but is not executable → status fail", async () => {
		...
		await fs.ensureDir(hooksSrc);
		await fs.writeFile(
			path.join(hooksSrc, "test-hook.sh"),
			"#!/bin/sh",
			"utf-8",
		);
		...
		const accessSpy = vi
			.spyOn(fs, "access")
			.mockImplementation((_path, _mode) => {
	...
	it("hook check: hook file exists and is executable → status ok", async () => {
		...
		await fs.ensureDir(hooksSrc);
		await fs.writeFile(
			path.join(hooksSrc, "exec-hook.sh"),
			"#!/bin/sh",
			"utf-8",
		);
	...
	it('skills section: no CLIs installed → shows "No CLIs installed" skip', async () => {
		...
		const skipCheck = skillSection.checks.find((c) =>
			c.label.includes("No CLIs installed"),
		);
		expect(skipCheck).toBeDefined();
	...
	it("installation section: shows correct date from manifest", async () => {
		...
		const dateCheck = installSection.checks.find((c) =>
			c.label.includes("Installed on"),
		);
	...
	it("installation section: shows installed CLIs list", async () => {
		...
		const cliListCheck = installSection.checks.find((c) =>
			c.label.includes("CLIs configured"),
		);
	...
	it("backup label format: T followed by colon-separated time (not dashes)", async () => {
		...
		await fs.writeFile(
			path.join(p.BACKUP_DIR, backupName, "f.md"),
			"x",
			"utf-8",
		);
	...
	it('hooks src path uses "own" subdir', async () => {
		...
		await fs.writeFile(
			path.join(wrongHooksSrc, "hook.sh"),
			"#!/bin/sh",
			"utf-8",
		);
	...
	it("countExpectedSkills: own skills do not require SKILL.md to be counted", async () => {
		...
		await fs.writeFile(
			path.join(ownSkills, "custom-tool", "README.md"),
			"# Tool",
			"utf-8",
		);
		...
		await fs.writeFile(
			path.join(p.CLAUDE_SKILLS, "custom-tool", "SKILL.md"),
			"# Tool",
			"utf-8",
		);
	...
	it("skill check: opt is found by exact CLI id match", async () => {
		...
		await fs.writeFile(
			path.join(p.OPENCODE_SKILLS, "react-19", "SKILL.md"),
			"# React 19",
			"utf-8",
		);
		...
		const result = await runDoctor();
		const skillSection = result.sections.find((s) => s.title === "Skills")!;
		const opencodeCheck = skillSection.checks.find(
			(c) => c.label.trim() === "opencode",
		);
	...
	it('hooks pathExists false → status fail with "not found" detail', async () => {
		...
		await fs.ensureDir(hooksSrc);
		await fs.writeFile(
			path.join(hooksSrc, "absent-hook.sh"),
			"#!/bin/sh",
			"utf-8",
		);
