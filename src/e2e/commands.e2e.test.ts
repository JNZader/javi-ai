...
import { createSandbox, removeSandbox, runCLI, countFiles } from './helpers.js'
...
describe('E2E: doctor', () => {
  ...
  it('doctor before install reports no installation found', async () => {
    ...
    const hasNoInstall = output.includes('No installation') || output.includes('fail') || output.includes('No CLIs')
...
describe('E2E: full lifecycle', () => {
  ...
  it('full lifecycle: install → doctor → update → uninstall → doctor', async () => {
    ...
    const hasNoInstall = doctorOutput2.includes('No installation') || doctorOutput2.includes('No CLIs')
    expect(hasNoInstall).toBe(true)
  }, 120_000)
})
...
describe('E2E: sync', () => {
  ...
  it('sync --project-dir generates CLAUDE.md in a project', async () => {
    ...
    const skillsDir = path.join(projectDir, '.ai-config', 'skills', 'test-skill')
  ...
  it('sync --dry-run does not write files', async () => {
    ...
    const result = await runCLI(['sync', '--project-dir', projectDir, '--dry-run'], sandbox)
    expect(result.exitCode).toBe(0)
  ...
  it('sync generates AGENTS.md for opencode target', async () => {
    ...
    const result = await runCLI(['sync', '--project-dir', projectDir, '--target', 'opencode'], sandbox)
    expect(result.exitCode).toBe(0)
