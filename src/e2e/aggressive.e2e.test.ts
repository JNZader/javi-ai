...
describe("E2E Aggressive: install verification", () => {
	...
	it("full claude install: every expected file exists", async () => {
		...
		expect(
			await fs.pathExists(
				path.join(skillsDir, "_shared", "persistence-contract.md"),
			),
		).toBe(true);
		expect(
			await fs.pathExists(
				path.join(skillsDir, "_shared", "engram-convention.md"),
			),
		).toBe(true);

		// Extensions: sdd-explore contains "Perspective Mode"
		const sddExplore = await fs.readFile(
			path.join(skillsDir, "sdd-explore", "SKILL.md"),
			"utf-8",
		);
		expect(sddExplore).toContain("Perspective Mode");

		// Extensions: sdd-apply contains "Per-Task" or "Rollback"
		const sddApply = await fs.readFile(
			path.join(skillsDir, "sdd-apply", "SKILL.md"),
			"utf-8",
		);
		expect(sddApply.includes("Per-Task") || sddApply.includes("Rollback")).toBe(
			true,
		);

		// Own skills installed
		expect(
			await fs.pathExists(path.join(skillsDir, "skill-creator", "SKILL.md")),
		).toBe(true);
		expect(
			await fs.pathExists(
				path.join(skillsDir, "obsidian-braindump", "SKILL.md"),
			),
		).toBe(true);
	...
	it("full opencode install: correct paths", async () => {
		...
		const manifest = await fs.readJSON(
			path.join(sandbox, ".javi-ai", "manifest.json"),
		);
	...
	it("multi-CLI install: all 3 paths populated", async () => {
		const result = await runCLI(
			["install", "--cli", "claude,opencode,qwen"],
			sandbox,
		);
		expect(result.exitCode).toBe(0);
		...
		expect(await countFiles(claudeSkills, "SKILL.md")).toBeGreaterThanOrEqual(
			30,
		);
...
describe("E2E Aggressive: extension model", () => {
	...
	it("upstream skill WITH extension has separator and both sections", async () => {
		...
		const basePath = (await fs.pathExists(overridePath))
			? overridePath
			: atlPath;
