...
const { FIXED_ASSETS_ROOT, FIXED_CLAUDE_PLUGINS_DEST } = vi.hoisted(() => {
  ...
  const o = require('os')
  return {
    FIXED_ASSETS_ROOT: p.join(o.tmpdir(), 'javi-ai-plugins-test-suite') as string,
    FIXED_CLAUDE_PLUGINS_DEST: p.join(o.tmpdir(), 'javi-ai-plugins-claude-dest-test') as string,
  }
})
  ...
  return {
    ...
    fileURLToPath: (url: string | URL) => {
      ...
      if (urlStr.includes('plugins') && !urlStr.includes('plugins.test')) {
        return nodePath.join(FIXED_ASSETS_ROOT, 'src', 'installer', 'plugins.js')
      }
    ...
    CLI_OPTIONS: [
      {
        ...
        skillsPath: nodePath.join(nodeOs.tmpdir(), 'javi-ai-plugins-skills-dest-test'),
        pluginsPath: FIXED_CLAUDE_PLUGINS_DEST,
        available: true,
      },
    ],
    MANIFEST_PATH: nodePath.join(nodeOs.tmpdir(), 'javi-ai-plugins-manifest-test.json'),
...
async function createPluginTree(): Promise<void> {
  ...
  await fs.writeFile(path.join(pluginDir, 'SKILL.md'), '# Test Plugin', 'utf-8')
  await fs.writeFile(path.join(pluginDir, 'commands', 'do.md'), '# Do command', 'utf-8')
  await fs.writeFile(
    path.join(pluginDir, 'connectors', 'tool.json'),
    JSON.stringify({ name: 'tool' }),
    'utf-8'
  )
  await fs.writeFile(path.join(pluginDir, 'references', 'deep.md'), '# Deep context', 'utf-8')

  // Plugin with CLI restriction (opencode only)
  const restrictedDir = path.join(FIXED_ASSETS_ROOT, 'own', 'plugins', 'opencode-only')
  ...
  await fs.writeFile(path.join(restrictedDir, 'SKILL.md'), '# OpenCode Only', 'utf-8')

  // Legacy skill (no manifest.json) — should be skipped
  const legacyDir = path.join(FIXED_ASSETS_ROOT, 'own', 'plugins', 'legacy-skill')
  await fs.ensureDir(legacyDir)
  await fs.writeFile(path.join(legacyDir, 'SKILL.md'), '# Legacy', 'utf-8')

  // Plugin with invalid manifest
  const invalidDir = path.join(FIXED_ASSETS_ROOT, 'own', 'plugins', 'bad-plugin')
  await fs.ensureDir(invalidDir)
  await fs.writeFile(path.join(invalidDir, 'manifest.json'), '{ broken json', 'utf-8')
  await fs.writeFile(path.join(invalidDir, 'SKILL.md'), '# Bad', 'utf-8')

  // Plugin with no compatible_tools (should install for all CLIs)
  const universalDir = path.join(FIXED_ASSETS_ROOT, 'own', 'plugins', 'universal-plugin')
  ...
  await fs.writeFile(path.join(universalDir, 'SKILL.md'), '# Universal', 'utf-8')
...
describe('readPluginManifest', () => {
  ...
  it('returns null for non-existent directory', async () => {
    const dir = path.join(FIXED_ASSETS_ROOT, 'own', 'plugins', 'does-not-exist')
    const manifest = await readPluginManifest(dir)
    expect(manifest).toBeNull()
  })
...
describe('validatePluginManifest', () => {
  ...
  it('detects missing description', () => {
    const errors = validatePluginManifest({ ...VALID_MANIFEST, description: '' })
    expect(errors).toContain('missing required field: description')
  })

  it('detects non-array dependencies', () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const errors = validatePluginManifest({ ...VALID_MANIFEST, dependencies: 'bad' as any })
    expect(errors).toContain('dependencies must be an array')
  })

  it('detects non-array compatible_tools', () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const errors = validatePluginManifest({ ...VALID_MANIFEST, compatible_tools: 'bad' as any })
    expect(errors).toContain('compatible_tools must be an array')
  })

  it('detects non-array triggers', () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const errors = validatePluginManifest({ ...VALID_MANIFEST, triggers: 'bad' as any })
...
describe('installPluginsForCLI', () => {
  ...
  it('installs valid plugin with all subdirectories', async () => {
    ...
    expect(await fs.pathExists(path.join(dest, 'commands', 'do.md'))).toBe(true)
    expect(await fs.pathExists(path.join(dest, 'connectors', 'tool.json'))).toBe(true)
    expect(await fs.pathExists(path.join(dest, 'references', 'deep.md'))).toBe(true)
  })

  it('installs universal plugin (no compatible_tools restriction)', async () => {
    await installPluginsForCLI('claude', false)
    expect(await fs.pathExists(path.join(FIXED_CLAUDE_PLUGINS_DEST, 'universal-plugin', 'SKILL.md'))).toBe(true)
  })

  it('does not install CLI-restricted plugin', async () => {
    await installPluginsForCLI('claude', false)
    expect(await fs.pathExists(path.join(FIXED_CLAUDE_PLUGINS_DEST, 'opencode-only'))).toBe(false)
  })
