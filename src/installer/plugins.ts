...
import type { CLI, PluginManifest, PluginFormat } from '../types/index.js'
...
export function detectFormat(dirPath: string): Promise<PluginFormat> {
  return fs.pathExists(path.join(dirPath, 'manifest.json')).then(exists =>
    exists ? 'plugin' : 'legacy-skill'
  )
}
 ...
 */
export async function readPluginManifest(dirPath: string): Promise<PluginManifest | null> {
 ...
 */
export async function installPluginsForCLI(cli: CLI, dryRun: boolean): Promise<string[]> {
