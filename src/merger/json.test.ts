...
describe("deepMerge (via mergeJsonFile)", () => {
	...
	it("array deduplication: primitive number values", async () => {
		const result = (await runMerge({ arr: [1, 2] }, { arr: [2, 3] })) as {
			arr: number[];
		};
	...
	it("array union: target items come first", async () => {
		const result = (await runMerge({ arr: ["a"] }, { arr: ["b"] })) as {
			arr: string[];
		};
...
describe("mergeJsonFile (integration)", () => {
	...
	it("nested merge correctness", async () => {
		...
		await fs.writeFile(
			tgtFile,
			JSON.stringify({ settings: { theme: "dark", lang: "en" } }),
			"utf-8",
		);
		await fs.writeFile(
			srcFile,
			JSON.stringify({ settings: { lang: "es", extra: true } }),
			"utf-8",
		);
		...
		const result = JSON.parse(await fs.readFile(tgtFile, "utf-8"));
		expect(result).toEqual({
			settings: { theme: "dark", lang: "es", extra: true },
		});
	});
	...
	it("array dedup correctness end-to-end", async () => {
		...
		const result = JSON.parse(await fs.readFile(tgtFile, "utf-8")) as {
			tags: string[];
		};
