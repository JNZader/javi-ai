		...
		CLI_OPTIONS: [
			{
				...
				pluginsPath: path.join(os.tmpdir(), ".claude-test", "plugins"),
				available: true,
			},
		],
...
describe("installSkillsForCLI() — integration", () => {
	...
	it("installs own skills", async () => {
		...
		expect(await fileExists(skillsDest, "skill-creator", "SKILL.md")).toBe(
			true,
		);
	});
	...
	it("installs ATL skills (sdd-*, branch-pr, issue-creation, skill-registry)", async () => {
		...
		expect(await fileExists(skillsDest, "issue-creation", "SKILL.md")).toBe(
			true,
		);
	});
	...
	it("delta override replaces ATL original SKILL.md", async () => {
		...
		const atlOriginal = await readFile(
			repoRoot,
			"upstream",
			"agent-teams-lite",
			"skills",
	...
	it("own skills override upstream when same name exists (skill-creator)", async () => {
		...
		const ownContent = await readFile(
			repoRoot,
			"own",
			"skills",
			"skill-creator",
	...
	it("_shared has ATL convention files", async () => {
		...
		expect(
			await fileExists(skillsDest, "_shared", "persistence-contract.md"),
		).toBe(true);
		expect(
			await fileExists(skillsDest, "_shared", "engram-convention.md"),
		).toBe(true);
		expect(
			await fileExists(skillsDest, "_shared", "openspec-convention.md"),
		).toBe(true);
		expect(await fileExists(skillsDest, "_shared", "sdd-phase-common.md")).toBe(
			true,
		);
	});
});
