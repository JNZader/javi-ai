...
import { findConfigDir, runSync } from "../commands/sync.js";
...
describe("runSync() — integration", () => {
	...
	it("merge mode preserves existing content outside markers", async () => {
		...
		const { onStep } = collectSteps();
		await runSync(
			{ target: "claude", mode: "merge", projectDir: tmpDir, dryRun: false },
			onStep,
		);
	...
	it("dry-run does not create files", async () => {
		...
		const { onStep } = collectSteps();
		await runSync(
			{ target: "claude", mode: "overwrite", projectDir: tmpDir, dryRun: true },
			onStep,
		);
	...
	it("target=all generates files for all CLIs", async () => {
		...
		const { onStep } = collectSteps();
		await runSync(
			{ target: "all", mode: "overwrite", projectDir: tmpDir, dryRun: false },
			onStep,
		);
