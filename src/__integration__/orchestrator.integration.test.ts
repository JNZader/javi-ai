...
describe('runInstall() orchestrator — integration', () => {
  ...
  it('installs configs: merges JSON files (settings.json)', async () => {
    ...
    const settings = await fs.readJson(path.join(FIXED_CLAUDE_CONFIG, 'settings.json'))
  ...
  it('creates backup before merging existing files', async () => {
    // Pre-existing file
    await fs.writeJson(path.join(FIXED_CLAUDE_CONFIG, 'settings.json'), { old: true })
  ...
  it('hooks are create-if-absent (existing hooks not overwritten)', async () => {
    ...
    await fs.writeFile(path.join(hooksDir, 'comment-check.sh'), '#!/bin/bash\necho custom', 'utf-8')
    ...
    const content = await fs.readFile(path.join(hooksDir, 'comment-check.sh'), 'utf-8')
