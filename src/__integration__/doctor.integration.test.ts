...
vi.mock('child_process', () => ({
  execFile: vi.fn((_cmd: string, args: string[], cb: Function) => {
    const tool = args?.[0] ?? ''
    if (tool === 'claude') cb(null, { stdout: '/usr/bin/claude\n', stderr: '' })
...
vi.mock('../installer/manifest.js', () => ({
  readManifest: async () => {
    ...
    if (await fse.pathExists(FIXED_MANIFEST)) return fse.readJson(FIXED_MANIFEST)
    return { version: '1.0.0', clis: [], installedAt: '', updatedAt: '', skills: {} }
  },
}))
...
describe('runDoctor() — integration', () => {
  ...
  it('reports no installation when manifest is missing', async () => {
    ...
    const installCheck = installSection!.checks.find(c => c.label.includes('No installation'))
  ...
  it('skills section shows count when skills installed', async () => {
    await fs.ensureDir(path.dirname(FIXED_MANIFEST))
    await fs.writeJson(FIXED_MANIFEST, {
      version: '1.0.0', clis: ['claude'],
      installedAt: '', updatedAt: '', skills: {},
    })

    // Create some skills
    for (const skill of ['react-19', 'typescript', 'sdd-apply']) {
      await fs.ensureDir(path.join(FIXED_CLAUDE_SKILLS, skill))
      await fs.writeFile(path.join(FIXED_CLAUDE_SKILLS, skill, 'SKILL.md'), `# ${skill}`)
    }

    const result = await runDoctor()
    const skillsSection = result.sections.find(s => s.title === 'Skills')
    expect(skillsSection).toBeDefined()
    const claudeSkills = skillsSection!.checks.find(c => c.label.includes('claude'))
  ...
  it('config section shows present/missing config files', async () => {
    ...
    const claudeConfig = configSection!.checks.find(c => c.label.includes('CLAUDE.md'))
