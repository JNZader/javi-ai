...
const FEATURES: { id: Feature; label: string; description: string }[] = [
  { id: 'skills',       label: 'Skills',           description: 'Upstream + own skills (SKILL.md files)' },
  { id: 'orchestrators',label: 'Orchestrators',     description: 'Domain orchestrators and agents' },
  { id: 'configs',      label: 'Configs',           description: 'CLI configurations (CLAUDE.md, opencode.json, etc.)' },
  { id: 'hooks',        label: 'Hooks',             description: 'Post-tool hooks (Claude only)' },
  { id: 'plugins',      label: 'Plugins',           description: 'Optional plugins (Claude only)' },
  { id: 'agents',       label: 'PSF Agents (90+)',  description: 'Full specialist agent library' },
]
...
export default function FeatureSelector({ selectedClis, onConfirm }: Props) {
  const [cursor, setCursor] = useState(0)
  const [selected, setSelected] = useState<Set<Feature>>(
    new Set(['skills', 'orchestrators', 'configs', 'hooks'])
  )
  ...
  return (
    <Box flexDirection="column">
      ...
      <Text color={theme.muted} dimColor>  CLIs: {selectedClis.join(', ')}</Text>
      ...
      >
        {FEATURES.map((f, i) => (
          <Box key={f.id}>
            ...
            <Text color={theme.muted} dimColor>  {f.description}</Text>
          </Box>
        ))}
      </Box>
